...
export default{
  ...
  methods: {
    _loadinterest () {
      this.$http.get('http://peicentapi.demo.sclonsee.com/v1/news/news-interest', {params: { type: 1 }}).then(response => {
        this.listtop = response.data.data
        console.log(this.listtop)
        this.cat_id = this.listtop[0].id
        this._loadData()
      }, response => {
        alert(response)
      })
    ...
    _loadData () {
      this.$http.get(this.geturl, {params: { cat_id: this.cat_id, type: 3, page: this.page, flag: 1, province_id: 51, city_id: 510100000000, region_id: 510104000000, street_id: 510104020000, community_id: 659004502528 }}).then(response => {
